...
CP_TEST_CASE("alignment playback plan preserves marker preamble and payload placement") {
    ...
    CP_REQUIRE(plan.audio.frame_count() == 5'403);
    const auto marker_amplitude = std::pow(10.0, -12.0 / 20.0);
    CP_REQUIRE_NEAR(plan.audio.samples[0], marker_amplitude, 1.0e-6);
    CP_REQUIRE_NEAR(plan.audio.samples[100], marker_amplitude, 1.0e-6);
    CP_REQUIRE_NEAR(plan.audio.samples[400], marker_amplitude, 1.0e-6);
    CP_REQUIRE_NEAR(plan.audio.samples[401], 0.0, 1.0e-9);
    CP_REQUIRE_NEAR(plan.audio.samples[5'400], 0.25, 1.0e-6);
    CP_REQUIRE_NEAR(plan.audio.samples[5'401], -0.5, 1.0e-6);
    CP_REQUIRE_NEAR(plan.audio.samples[5'402], 0.75, 1.0e-6);
}
...
CP_TEST_CASE("alignment playback gain applies equally to markers and payload") {
    ...
    CP_REQUIRE_NEAR(
        plan.audio.samples[0],
        std::pow(10.0, -12.0 / 20.0) * gain,
        1.0e-6);
    CP_REQUIRE_NEAR(plan.audio.samples[5'400], source.samples[0] * gain, 1.0e-6);
    CP_REQUIRE_NEAR(plan.audio.samples[5'401], source.samples[1] * gain, 1.0e-6);
}
...
CP_TEST_CASE("payload alignment uses source start plus median marker latency") {
    ...
    CP_REQUIRE(samples_near(result.audio.samples, source));
